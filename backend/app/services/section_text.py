...
def should_render_section_title(section_title: str, content_text: dict[str, Any] | None) -> bool:
    ...
    for value in visible_values:
        ...
        if title_key and value_key and (title_key == value_key or title_key in value_key):
            return False
...
def sanitize_section_visual_numbering(text: str) -> str:
    ...
    value = re.sub(r"(?:part|chapter)\s*0?\d{1,3}\s*(?:编号|序号)", "章节转场", value, flags=re.IGNORECASE)
    value = re.sub(rf"模块\s*{_ORDINAL_TOKEN_RE}", "章节", value)
    value = re.sub(rf"[「『“\"']\s*{_ORDINAL_TOKEN_RE}\s*[」』”\"']", "", value)
    value = value.replace("编号", "章节转场").replace("序号", "章节转场")
    value = re.sub(r"章节转场\s*[与和]\s*主标题", "主标题", value)
    value = re.sub(r"章节标题\s*/\s*章节转场\s*/\s*转场氛围", "章节标题/转场氛围", value)
